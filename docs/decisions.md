...
## D6: RMLVO Support (Implemented)

**Decision**: RMLVO compilation is implemented via `ctx.NewKeymapFromNames()`.

**RMLVO** = Rules + Model + Layout + Variant + Options

**Implementation** (in `rules.go`):

- Rules file parser that understands XKB rules format
- RMLVO → KcCGST (Keycodes, Compat, Geometry, Symbols, Types) resolution
- Component file loading with `include`/`augment` directive support
- Section extraction and merging

**Usage**:

```go
// Build US layout keymap from system XKB data
keymap, err := ctx.NewKeymapFromNames(&xkb.RuleNames{Layout: "us"})
